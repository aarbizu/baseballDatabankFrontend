...
import org.aarbizu.baseballDatabankFrontend.config.csvHome
import org.slf4j.LoggerFactory
import java.io.File

class DataLoader(private val db: DBProvider) {
    private val log = LoggerFactory.getLogger(this.javaClass)

    private val fileLoaders = mapOf(
    ...
    fun loadAllFiles() {
        ...
        getCsvFiles().forEach {
            log.info("loading $it")
            fileLoaders[it.nameWithoutExtension]?.load(db, it).also {
                count += 1
            }
        }
    ...
    fun getCsvFiles(): List<File> {
        return File(csvHome).walkTopDown().filter {
            it.isFile && it.extension == "csv"
        }.toList()
    }
